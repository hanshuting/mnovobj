...
plot_mikki_box_pair(rest_time_center_all,[0 0 0],cc.orange,p)
ylabel('rest time center (s)');
legend('off')

% rest time edge
subplot(2,5,5); hold on;
plot_mikki_box_pair(rest_time_edge_all,[0 0 0],cc.orange,p)
ylabel('rest time edge (s)');
legend('off')

% run time center
subplot(2,5,6); hold on;
plot_mikki_box_pair(run_time_center_all,[0 0 0],cc.orange,p)
ylabel('run time center (s)');
legend('off')

% run time edge
subplot(2,5,7); hold on;
plot_mikki_box_pair(run_time_edge_all,[0 0 0],cc.orange,p)
ylabel('run time edge (s)');
legend('off')

% cornere rest time
subplot(2,5,8); hold on;
plot_mikki_box_pair(corner_rest_time_all,[0 0 0],cc.orange,p)
ylabel('corner rest time (s)');
legend('off')

% speed
subplot(2,5,9); hold on;
plot_mikki_box_pair(spd_all,[0 0 0],cc.orange,p)
ylabel('speed');
legend('off')

% num stop
subplot(2,5,10); hold on;
plot_mikki_box_pair(num_stop_all,[0 0 0],cc.orange,p)
ylabel('num stop');
